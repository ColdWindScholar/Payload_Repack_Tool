...
import rangelib
import sparse_img
from sign_payload import private_der_to_pem
...
def GetKeyPasswords(keylist):
    ...
    for k in sorted(keylist, key=lambda x: x if x is not None else ""):
        ...
        if k in SPECIAL_CERT_STRINGS or k is None:
            ...
            continue
        try:
            private_der_to_pem(k + OPTIONS.private_key_suffix, "/dev/null" if os.name == 'posix' else 'null')
            # Definitely an unencrypted key.
            no_passwords.append(k)
        except TypeError:
            try:
                private_der_to_pem(k + OPTIONS.private_key_suffix, "/dev/null" if os.name == 'posix' else 'null', password=b'')
                # Encrypted key with empty string as password.
                key_passwords[k] = ''
            except Exception as e:
                if 'Error decrypting key' in e:
