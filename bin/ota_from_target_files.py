...
import sys
import zipfile
from sign_payload import payload_sign, private_der_to_pem
import care_map_pb2
import common
import ota_utils
  ...
  """
    ...
    def __init__(self):
        if OPTIONS.payload_signer is None:
            ...
            self.signing_key = common.MakeTempFile(prefix="key-", suffix=".key")
            if isinstance(pw, str):
                pw = pw.encode('utf-8')
            private_der_to_pem(private_key, self.signing_key, password=pw if pw else None)
    ...
    def _GetMaximumSignatureSizeInBytes(signing_key):
        ...
        cmd = ["delta_generator_x86_64" if arch == 'x86_64' else "delta_generator_aarch64",
               f"--out_maximum_signature_size_file={out_signature_size_file}", f"--private_key={signing_key}"]
    ...
    def Sign(self, in_file):
        ...
        if OPTIONS.payload_signer:
            cmd = [OPTIONS.payload_signer] + OPTIONS.payload_signer_args + ['-in', in_file, '-out', out_file]
            common.RunAndCheckOutput(cmd)
        else:
          # nah rsa do not support pkcs8
          payload_sign(self.signing_key, in_file, out_file)
        return out_file
    ...
    if not OPTIONS.no_signing or generate_ab:
        ...
        if not OPTIONS.payload_signer:
            ...
            if not os.path.exists(signapk_abs_path):
                raise common.ExternalError(
                    "Failed to find sign apk binary {} in search path {}. Make sure the correct search path is passed via -p".format(
                        OPTIONS.signapk_path, OPTIONS.search_path))
