#!/bin/env python3
import os.path
from bin.sign_payload import payload_sign
...
def main():
    ...
    system(f'./bin/delta_generator_{arch} -out_file=output/unsigned-payload.bin -partition_names={name_list} -new_partitions={file_list} -dynamic_partition_info_file=./META/dynamic_partitions_info.txt -new_postinstall_config_file=./META/postinstall_config.txt')
    ...
    payload_sign('key/testkey.key', 'output/sig_hash.bin', 'output/signed_hash.bin')
    payload_sign('key/testkey.key', 'output/sig_metadata.bin', 'output/signed_metadata.bin')
